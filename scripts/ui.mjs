...
// The shared UI kit: one set of tokens, primitives and motion for every app.
// Apps carry a copy inline between the jnssn-ui fences rather than linking it;
// `node scripts/build.mjs --sync-ui` writes it out. See CLAUDE.md, "The UI kit".

import { hueFor } from './icon.mjs';
import { createHash } from 'node:crypto';

// Must run in <head>, before first paint: any later and a light-set site on a
// dark phone paints dark, then flips.
...
// Light on bare :root, dark overriding only the values. A colour defined only
// inside a media query never applies in the un-stamped state.
...
/* progress: dots up to 14 rounds, a bar past that */
...
/* Sheets FADE; they never slide, and the box is sized in svh rather than
   inset:0 — WebKit mis-places a transformed or clipped box inside a <dialog>.
   Both are load-bearing; see CLAUDE.md, "The UI kit". */
dialog.sheet{
...
/* board: set --cols; the cells size themselves */
...
/* stat: for the one or two figures that ARE the point of the screen */
...
// Small on purpose: only what every app needs and nobody should re-derive.
export const UI_JS = `
(function(){
  ...
  function openSheet(el){
    ...
    // Not {once:true}: a tap on a control inside the sheet bubbles to the
    // dialog and would consume it, leaving tap-outside dead for that open.
    if(el.dataset.backdrop !== '1'){
      el.dataset.backdrop = '1';
  ...
  function dragSheet(el){
    ...
    body.addEventListener('touchmove', function(e){
      ...
      if(dy <= 0){
        if(body.scrollTop > 0){ dragging = false; body.style.transition = ''; body.style.transform = ''; }
        return;
      }
      e.preventDefault();
