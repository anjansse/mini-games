...
dialog.sheet{
  border:0;padding:0;background:transparent;max-width:480px;width:100%;
  margin:auto auto 0;max-height:88svh;
}
...
dialog.sheet>.body{
  ...
  animation:sheet-up var(--slow) var(--ease) both;
}
...
@media (min-width:520px){
  ...
  dialog.sheet>.body{border-radius:var(--r-lg);border-bottom:1px solid var(--line);animation-name:rise}
...
(function(){
  ...
  function openSheet(el){
    ...
    if(body){
      // A sheet reopens at the top, never where it was left. Without this a
      // sheet that was scrolled once comes back scrolled, and its heading and
      // first options sit above the fold looking as though they are missing.
      // This has to run AFTER showModal: a closed <dialog> is display:none,
      // and assigning scrollTop to a display:none element does nothing.
      body.style.transition = ''; body.style.transform = ''; body.style.animation = '';
      body.scrollTop = 0;
    }
  ...
  function closeSheet(el){
    ...
    // A transition, not the keyframe reversed: a keyframe always starts from
    // translateY(0), so closing a sheet the finger had already dragged to
    // 150px snapped it back up first and then played it down — the double
    // movement. A transition runs from wherever it actually is.
